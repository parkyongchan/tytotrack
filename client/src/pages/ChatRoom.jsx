...
const CHAT_T = {
  ko: {
    ...
    sending: '', send: '', failed: '실패', sent: '성공', waiting: '대기중', gw: 'GW접수',
    ...
    deleteFail: '삭제 실패: ',
    unreadTotal: '미읽음', markAllRead: '모두 읽음 처리',
    selectMode: '선택 삭제', selectCancel: '취소', selectedCount: '개 선택됨',
    selectAll: '전체 선택', deselectAll: '전체 해제',
    deleteRoomsBtn: '선택한 채팅방 삭제',
    deleteRoomsConfirm: '선택한 {n}개 채팅방의 모든 메시지가 삭제됩니다. 계속하시겠습니까?',
    deleteRoomsSuccess: '{n}개 채팅방 삭제 완료',
    deleteRoomsFail: '삭제 실패',
    selectModeHint: '카드를 클릭하여 선택/해제',
  },
  en: {
    title: 'CHAT ROOM', subtitle: 'Satellite MSG', activeChannels: 'active channels',
    ...
    unreadTotal: 'Unread', markAllRead: 'Mark all as read',
    selectMode: 'Select & Delete', selectCancel: 'Cancel', selectedCount: ' selected',
    selectAll: 'Select all', deselectAll: 'Deselect all',
    deleteRoomsBtn: 'Delete selected rooms',
    deleteRoomsConfirm: 'All messages in {n} selected room(s) will be deleted. Continue?',
    deleteRoomsSuccess: '{n} room(s) deleted',
    deleteRoomsFail: 'Delete failed',
    selectModeHint: 'Click cards to select/deselect',
  },
  ja: {
    ...
    deleteFail: '削除失敗: ',
    unreadTotal: '未読', markAllRead: 'すべて既読',
    selectMode: '選択削除', selectCancel: 'キャンセル', selectedCount: '個選択中',
    selectAll: 'すべて選択', deselectAll: '選択解除',
    deleteRoomsBtn: '選択したルームを削除',
    deleteRoomsConfirm: '選択した{n}個のルームのすべてのメッセージが削除されます。続行しますか？',
    deleteRoomsSuccess: '{n}個のルームを削除しました',
    deleteRoomsFail: '削除失敗',
    selectModeHint: 'カードをクリックして選択/解除',
  },
};
...
// 미읽음 뱃지 시스템
// ═══════════════════════════════════════════════════════
const getLastVisit = (imei) => {
  return localStorage.getItem(`chat_lastVisit_${imei}`) || '00000000000000';
};

const setLastVisit = (imei) => {
  const now = new Date();
  ...
  localStorage.setItem(`chat_lastVisit_${imei}`, stamp);
  return stamp;
};

const getUnreadCount = (imei, msgs) => {
  const lastVisit = getLastVisit(imei);
...
const isUnread = (m, imei) => {
  if (m._type !== 'snd') return false;
  const lastVisit = getLastVisit(imei);
  return (m.regDate || '') > lastVisit;
};
...
export default function ChatRoom({ devices = [] }) {
  ...
  const [selectedRooms, setSelectedRooms] = useState(new Set());  // imei Set
  ...
  const fetchMessages = useCallback(async () => {
    try {
      ...
      const rcv = (Array.isArray(rcvRes.data) ? rcvRes.data : [])
        .map(m => ({
          ...m,
          imei: pickField(m, 'mEsn', 'mesn', 'm_esn'),
          title: pickField(m, 'mTitle', 'mtitle', 'm_title'),
          text: pickField(m, 'mMemo', 'mmemo', 'm_memo'),
          status: pickField(m, 'msgStatus', 'msg_status'),
          regDate: pickField(m, 'regDate', 'reg_date'),
          idx: m.mid,
        }))
        .filter(m => isSuperAdmin || myImeis.includes(m.imei));
      
      setSndMessages(snd);
      setRcvMessages(rcv);
    } catch (e) {
      console.error('fetchMessages 실패:', e);
    }
  ...
  ])].filter(imei => imei && (isSuperAdmin || myImeis.includes(imei)));
  ...
  const _trigger = unreadVersion;
  const totalUnread = chatDevices.reduce(
    (sum, { imei, msgs }) => sum + getUnreadCount(imei, msgs),
    0
  );
  ...
  const toggleRoomSelection = (imei) => {
  ...
  const selectAllRooms = () => {
    setSelectedRooms(new Set(chatDevices.map(c => c.imei)));
  };
  ...
  const handleDeleteRooms = async () => {
    ...
    const confirmMsg = t.deleteRoomsConfirm.replace('{n}', selectedRooms.size);
    if (!confirm(confirmMsg)) return;
    
    setDeleting(true);
    try {
      const imeis = Array.from(selectedRooms);
      const res = await api.delete('/chat/rooms', {
        data: { imeis }
      });
      
      const successMsg = t.deleteRoomsSuccess.replace('{n}', selectedRooms.size);
      alert(`${successMsg}\n(메시지 ${res.data?.total || 0}개 삭제됨)`);
      
      // 삭제된 채팅방의 lastVisit 정리
      imeis.forEach(imei => {
        localStorage.removeItem(`chat_lastVisit_${imei}`);
      });
      ...
      alert(`${t.deleteRoomsFail}: ${msg}`);
    } finally {
      setDeleting(false);
    }
  };

  const enterRoom = (imei, alias) => {
    if (selectMode) {
      // 선택 모드에선 카드 클릭 = 선택 토글
      toggleRoomSelection(imei);
      return;
    }
    setLastVisit(imei);
  ...
  const handleMarkAllRead = () => {
    chatDevices.forEach(({ imei }) => setLastVisit(imei));
    setUnreadVersion(v => v + 1);
  };
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'linear-gradient(180deg,#0a1628 0%,#0d1e38 100%)' }}>
      {/* 헤더 */}
      <div style={{ padding: '14px 24px', borderBottom: '1px solid rgba(0,212,240,.15)', display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0, background: 'rgba(10,20,40,.8)', backdropFilter: 'blur(10px)', flexWrap: 'wrap' }}>
        ...
        {!selectMode && totalUnread > 0 && (
          <div style={{ 
            ...
            <span style={{ fontSize: '10px', color: '#fff', fontWeight: '600', letterSpacing: '0.5px' }}>{t.unreadTotal}</span>
            <span style={{ 
              fontSize: '13px', 
              color: '#fff', 
              fontWeight: '700',
        ...
        {selectMode && (
          <>
            <div style={{ 
              ...
              {selectedRooms.size}{t.selectedCount}
            </div>
            <span style={{ fontSize: '10px', color: '#6b8fae' }}>{t.selectModeHint}</span>
          </>
        )}
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <>
              ...
              {totalUnread > 0 && (
                <button onClick={handleMarkAllRead}
                  title={t.markAllRead}
                  style={{ padding: '6px 10px', background: 'rgba(245,158,11,.15)', border: '1px solid rgba(245,158,11,.3)', borderRadius: '8px', color: '#f59e0b', fontSize: '10px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>
                  <IconCheckCircle size={12} color="#f59e0b" /> {t.markAllRead}
                </button>
              )}
              ...
              {/* 선택 삭제 모드 진입 버튼 — SUPER_ADMIN만 */}
              {isSuperAdmin && chatDevices.length > 0 && (
                <button onClick={enterSelectMode}
                  style={{ padding: '6px 12px', background: 'rgba(239,68,68,.12)', border: '1px solid rgba(239,68,68,.3)', borderRadius: '8px', color: '#ef4444', fontSize: '11px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}>
                  <IconTrash size={12} color="#ef4444" /> {t.selectMode}
                </button>
              )}
          ...
          {selectMode && (
            <>
              <button onClick={selectedRooms.size === chatDevices.length ? deselectAllRooms : selectAllRooms}
                style={{ padding: '6px 12px', background: 'rgba(0,212,240,.12)', border: '1px solid rgba(0,212,240,.3)', borderRadius: '8px', color: '#00d4f0', fontSize: '11px', fontWeight: '700', cursor: 'pointer' }}>
                {selectedRooms.size === chatDevices.length ? t.deselectAll : t.selectAll}
              </button>
              <button onClick={handleDeleteRooms} disabled={selectedRooms.size === 0 || deleting}
                style={{ 
                  padding: '6px 14px', 
                  background: selectedRooms.size === 0 ? 'rgba(255,255,255,.06)' : 'linear-gradient(135deg,#ef4444,#dc2626)', 
                  border: 'none', 
                  borderRadius: '8px', 
                  color: '#fff', 
                  fontSize: '11px', 
                  fontWeight: '700', 
                  cursor: selectedRooms.size === 0 ? 'not-allowed' : 'pointer',
                  opacity: selectedRooms.size === 0 ? 0.4 : 1,
                  display: 'flex', alignItems: 'center', gap: '5px',
                  boxShadow: selectedRooms.size === 0 ? 'none' : '0 4px 12px rgba(239,68,68,.4)'
                }}>
                <IconTrash size={12} color="#fff" /> 
                {deleting ? '삭제중...' : `${t.deleteRoomsBtn} (${selectedRooms.size})`}
              </button>
              <button onClick={exitSelectMode}
                style={{ padding: '6px 12px', background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.1)', borderRadius: '8px', color: '#6b8fae', fontSize: '11px', fontWeight: '700', cursor: 'pointer' }}>
                {t.selectCancel}
              </button>
            </>
          )}
        ...
        <div style={{ padding: '10px 24px', borderBottom: '1px solid rgba(0,212,240,.1)', maxHeight: '180px', overflowY: 'auto', flexShrink: 0 }}>
          {searchResults.map((r, i) => {
            ...
            const highlighted = idx >= 0
              ...
              : text;
            return (
              <div key={i} onClick={() => enterRoom(r.imei, r.alias)}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexWrap: 'wrap', gap: '16px', alignContent: 'flex-start' }}>
        {chatDevices.length === 0 ? (
          ...
          const unread = getUnreadCount(imei, msgs);
          const hasSOS = msgs.some(m => m.eventcode === '4' && isUnread(m, imei));
          const lastMsg = msgs[msgs.length - 1];
          const isSelected = selectedRooms.has(imei);

          return (
            <div key={imei} onClick={() => enterRoom(imei, alias)}
              style={{ 
                width: '260px', 
                background: isSelected 
                  ? 'linear-gradient(135deg,rgba(239,68,68,.15),rgba(220,38,38,.1))'
                  : 'linear-gradient(135deg,rgba(20,35,60,.95),rgba(14,26,46,.95))', 
                border: `${isSelected ? '2px' : '1px'} solid ${
                  isSelected ? '#ef4444' : (hasSOS ? 'rgba(239,68,68,.4)' : 'rgba(0,212,240,.2)')
                }`, 
                borderRadius: '16px', 
                padding: '16px', 
                cursor: 'pointer', 
                transition: 'all .2s', 
                boxShadow: isSelected ? '0 4px 20px rgba(239,68,68,.4)' : '0 4px 20px rgba(0,0,0,.3)', 
                flexShrink: 0,
                position: 'relative',
              }}
              onMouseEnter={e => { 
                if (!isSelected) {
                  ...
                  e.currentTarget.style.boxShadow = '0 8px 30px rgba(0,212,240,.15)'; 
                  e.currentTarget.style.border = '1px solid rgba(0,212,240,.5)'; 
                }
              }}
              onMouseLeave={e => { 
                if (!isSelected) {
                  ...
                  e.currentTarget.style.border = `1px solid ${hasSOS ? 'rgba(239,68,68,.4)' : 'rgba(0,212,240,.2)'}`; 
                }
              }}>
              ...
              {selectMode && (
                <div style={{
                  ...
                  height: '24px',
                  borderRadius: '50%',
                  background: isSelected ? '#ef4444' : 'rgba(0,0,0,.4)',
                  border: `2px solid ${isSelected ? '#ef4444' : 'rgba(255,255,255,.3)'}`,
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                  zIndex: 10,
                  boxShadow: isSelected ? '0 0 12px rgba(239,68,68,.6)' : 'none',
                }}>
                  {isSelected && (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                {last4.length === 0 ? (
                  ...
                  const text = isRcv 
                    ? (m.text || m.mMemo || m.mmemo || m.m_memo || '')
                    : (m.memo || '');
                  
                  const unreadDot = !isRcv && isUnread(m, imei);
                  
                  let statusIcon = null;
                  if (isRcv) {
                    const st = m.status || m.msgStatus || m.msg_status || '0';
                    if (st === '0') statusIcon = '⏳';
                    else if (st === '1') statusIcon = '📡';
                    else if (st === '2') statusIcon = '✅';
                    else if (st === '3') statusIcon = '❌';
                  }
                  
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <span style={{ fontSize: '8px', color: isRcv ? '#00d4f0' : '#a78bfa', flexShrink: 0 }}>{isRcv ? '▶' : '◀'}</span>
                      <span style={{ 
                        fontSize: '10px', 
                        color: isRcv ? '#e8f4ff' : '#c4b5fd',
                        opacity: !isRcv && !unreadDot ? 0.55 : 1,
                      ...
                      {statusIcon && !selectMode && (
                        <span style={{ fontSize: '10px', flexShrink: 0 }}>{statusIcon}</span>
                      )}
                    </div>
                  );
                })}
...
function ChatRoomFull({ imei, alias, messages, onBack, onRefresh, isSuperAdmin, devices }) {
  ...
  const deleteMessage = async (idx, type) => {
    ...
    try {
      const res = await api.delete(`/chat/${type}/${idx}`);
      console.log('삭제 성공:', res.data);
      alert('삭제되었습니다.');
      onRefresh();
    } catch (e) {
      console.error('삭제 실패:', e);
      const msg = e.response?.data?.message || e.response?.statusText || e.message || '알 수 없는 에러';
      alert(t.deleteFail + msg + ` (status: ${e.response?.status})`);
    }
  };
  ...
  const getStatusIcon = (m) => {
    const st = m.status || m.msgStatus || m.msg_status || '0';
    if (st === '3') return <span style={{ fontSize: '9px', color: '#ef4444', display: 'flex', alignItems: 'center', gap: '3px' }}><IconXCircle size={10} color="#ef4444" /> {t.failed}</span>;
    if (st === '2') return <span style={{ fontSize: '9px', color: '#10b981', display: 'flex', alignItems: 'center', gap: '3px' }}><IconCheckCircle size={10} color="#10b981" /> {t.sent}</span>;
    if (st === '1') return <span style={{ fontSize: '9px', color: '#00d4f0', display: 'flex', alignItems: 'center', gap: '3px' }}><IconSignal size={10} color="#00d4f0" /> {t.gw}</span>;
    return <span style={{ fontSize: '9px', color: '#f59e0b', display: 'flex', alignItems: 'center', gap: '3px' }}><IconClock size={10} color="#f59e0b" /> {t.waiting}</span>;
  };
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'linear-gradient(180deg,#0a1628 0%,#0d1e38 100%)' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {messages.length === 0 ? (
          ...
          const isRcv = m._type === 'rcv';
          const displayText = isRcv 
            ? (m.text || m.mMemo || m.mmemo || m.m_memo || '')
            : (m.memo || '');
          const displayTitle = isRcv
            ? (m.title || m.mTitle || m.mtitle || m.m_title || '')
            : (m.title || '');
            ...
            <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: isRcv ? 'flex-end' : 'flex-start' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: '8px', flexDirection: isRcv ? 'row-reverse' : 'row', maxWidth: '70%' }}>
                <div style={{
                  ...
                  {displayTitle && (
                    <div style={{ fontSize: '11px', fontWeight: '700', color: isRcv ? '#0a4a5a' : '#f70202', marginBottom: '4px', paddingBottom: '4px', borderBottom: isRcv ? '1px solid rgba(0,0,0,.15)' : '1px solid rgba(255,255,255,.15)' }}>
                      <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <IconSignal size={14} color={isRcv ? '#0a4a5a' : '#f70202'} /> {displayTitle}
                      </span>
                    </div>
                  )}
                  <div>{displayText || '(내용 없음)'}</div>
                  {!isRcv && m.rimei && (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: isRcv ? 'flex-end' : 'flex-start', gap: '3px', flexShrink: 0 }}>
                  ...
                  {isRcv && (m.status === '3' || m.msgStatus === '3' || m.msg_status === '3') && (
                    <button onClick={async () => {
                      try { await api.put(`/chat/rcv/${m.idx || m.mid}/retry`); onRefresh(); }
                  ...
                  {isSuperAdmin && (
                    <button onClick={() => deleteMessage(m.idx || m.mid, m._type)}
